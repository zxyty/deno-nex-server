...
class MiddlewareRoute {
    ...
    protected getMatchedRoute = (url: string, method: string) => {
        const methodMatchedRoute = this.handler
            .filter(c => String(c.method).toLocaleLowerCase() === String(method).toLocaleLowerCase());
        let params = {};
    ...
    static = (path: string) => {
        ...
        if (path.startsWith("http:") || path.startsWith("https:")) {
            ...
            if (import.meta.url.startsWith("http:") || import.meta.url.startsWith("https:")) {
                ...
                this.serverStatic.push(stdPath.join(stdPath.dirname(import.meta.url), path));
            } else {
                try {
                    throw new Error();
                    ...
                    const enrtyFileStack = error.stack.split('at ').map((c: string) => c.trim()).pop() as string;
    
                    // in win32 such as file:///D:/xxx/xxx/a.ts:8:3
                    const entryFileDir = stdPath.dirname(stdPath.fromFileUrl(enrtyFileStack.replace(/\:[0-9]+\:[0-9]+/, '')));
